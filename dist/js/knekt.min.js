function initAbout(){var a=$("#about-button"),b=$("#about-close-button"),c=$("#about-overlay"),d=function(a){!c.is(":visible")||$(a.target).closest("#about-overlay").length||$(a.target).is("#about-button")||(c.hide(),$(document).unbind("click",d))};a.click(function(){c.show();$(document).click(d)});b.click(function(){c.hide();$(document).unbind("click",d)});$(document).keyup(function(a){27===a.keyCode&&(c.hide(),$(document).unbind("click",d))})};var appConfig=appConfig||{};appConfig.CRYPTO_TEXT_PATH="res";appConfig.TEXT_FILE_COUNT=102;$(function(){initAppState();initInterface();initBigramFactorMngr();initNGramTablesMngr();initStatChartsMngr();initHomeMngr();initViewMngr();initSubstMngr();initTransMngr();initVigMngr();initAbout()});function AppState(a,b){var c=a,d=b,e="",f={};this.getLang=function(){return c};this.getFileName=function(){return d};this.getFileText=function(){return e};this.fileIsSelected=function(){return void 0!==d};this.applySettings=function(){var a=$("#lang-select"),b=$("#file-select");a.hasClass("changed-setting")&&(c=a.val(),a=new CustomEvent("languageChange",{detail:c}),document.body.dispatchEvent(a));b.hasClass("changed-setting")&&(d=b.val(),h());clearSettingIndications();$("#apply-button").prop("disabled",
...
$("#util-bar").mCustomScrollbar({axis:"y"})})};function getNGramCount(a,b,c){if("string"!==typeof a)throw TypeError("text must be a string");if("number"!==typeof b)throw TypeError("n must be a number");if(1>b)throw RangeError("n must be greater than or equal to one");if("number"!==typeof c)throw TypeError("count must be a number");if(0>c)throw RangeError("count must be non-negative");a=a.toUpperCase();for(var d={},e="",f=0;f<=a.length-b;f++)e=a.slice(f,f+b),d[e]=d[e]?d[e]+1:1;a=[];for(e in d)d.hasOwnProperty(e)&&a.push({ngram:e,num:d[e]});a.sort(function(a,
b){return b.num-a.num});return a.slice(0,c)};function updateTextNGgramTables(a){var b={ngrams:getNGramCount(a,2,10),label:"Bigram",numType:"Count"};a={ngrams:getNGramCount(a,3,10),label:"Trigram",numType:"Count"};handlebarExec("#ngram-table-template","#text-bigram-table",b);handlebarExec("#ngram-table-template","#text-trigram-table",a)}function plotLanguageNGramTables(a,b){handlebarExec("#ngram-table-template","#language-bigram-table",a);handlebarExec("#ngram-table-template","#language-trigram-table",b)}
...
"No file loaded")},initStatChartsMngr=function(){document.body.addEventListener("languageChange",function(a){plotGraphs(appState.getLang())});document.body.addEventListener("fileChange",function(a){plotGraphs(appState.getLang())});plotGraphs(appState.getLang())};function handlebarAlphabetDivs(a){if("object"!==typeof a)throw Error("alphabet must be an object");var b={letters:[]},c;for(c in a)a.hasOwnProperty(c)&&b.letters.push(c.toUpperCase());handlebarExec("#orig-alphabet-hbs","#plain-cryptobet .normal-alphabet",b);handlebarExec("#orig-alphabet-hbs","#crypto-plainbet .normal-alphabet",b)}
...
function handlebarMessageDivs(a,b){if("undefined"==typeof a.length||"undefined"==typeof a.substr)throw Error("a message needs a length");if("object"!==typeof b)throw Error("alphabet must be an object");for(var c=Math.ceil(a.length/60),d={row:[]},e=0;e<c;++e){var f="PLACEHOLDER";a&&(f=a.substr(60*e,60));d.row.push({encrypted:f,decrypted:decryptLine(f,b)})}handlebarExec("#message-divs-hbs","#message-divs",d)};function initAlphabet(a){if(!Array.prototype.reduce)throw Error("Browser does not have array.reduce");return getAlphabet(a).toUpperCase().split("").reduce(function(a,c){a[c]="_";return a},{})}
...
function invertObject(a){if("object"!==typeof a)throw Error("can't invert non-objects");var b={},c;for(c in a)a.hasOwnProperty(c)&&(b[a[c]]=c);return b}jQuery.fn.toInnerHTMLArray=function(){var a=[];this.each(function(b,c){a.push(c.innerHTML.trim(""))});return a};jQuery.fn.isEmpty=function(){return""==jQuery.trim(this.html())};function initSubstMngr(){substResetTool();document.body.addEventListener("fileChange",function(a){substResetTool("fileChange",a.detail)});document.body.addEventListener("languageChange",function(){substResetTool("resetDecrypted")})}
...
function initTransMngr(){document.body.addEventListener("fileChange",function(a){$("#trans-result").html("");generateNewColumns()});dragula([document.querySelector("#column-container")],{revertOnSpill:!1,direction:"horizontal"}).on("drop",function(){updateResultText()})};function memoize(a){var b={};return function(){var c=String(Hashcode.value(JSON.stringify(arguments))>>>0);return b[c]=b[c]||a.apply(this,arguments)}}function getValuesFromObject(a){if(null===a||"object"!==typeof a)throw new TypeError("Not an object!");var b=[],c;for(c in a)Object.prototype.hasOwnProperty.call(a,c)&&b.push(a[c]);return b}
...
handlebarVigenereKey=function(a,b){if("number"!==typeof a)throw Error("keyLength must be an number");for(var c={keyLetter:[]},d=0;d<a;++d)c.keyLetter.push("A");b=b||{};d=$("#vigenere-key-hbs").html();d=handlebarCompile(d);c=handlebarBuild(d,c);d=$("#vig-key");if(!b.resetKey){var e=$("<div></div>");e.html(c);var f=d.children();e.children().each(function(a,b){var c=f.get(a);c&&($(b).html(c.innerHTML),$(b).addClass(c.className))});c=e.html()}d.html(c)};var caesarLookUp=function(a,b,c){if("string"!==typeof a||"string"!==typeof b||"string"!==typeof c)throw new TypeError("Argument is not a string");if(""===a||""===b||""===c)throw Error("Argument is empty string");var d=c.indexOf(a);if(-1===d)return console.log("vig:Cryptoletter not in alphabet"),"?";b=c.indexOf(b);if(-1===b)throw Error("Key is not in the alphabet");a=c.indexOf(a,b);return c[-1===a?c.length-b+d:a-b]},caesarReverseLookUp=function(a,b,c){a=c.indexOf(a);b=c.indexOf(b);return c[(a+b)%c.length]},
